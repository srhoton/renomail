...
func resolveDisplay(ctx context.Context, db *sql.DB, acctID string, inboxMbox int64) (string, error) {
	...
	recip, err := singleString(ctx, db, `
		...
		WHERE m.mailbox = ? AND r.type = 0
		GROUP BY a.address ORDER BY COUNT(*) DESC LIMIT 1`, inboxMbox)
	if err != nil {
...
// seconds), newest first, mapped to body-less items.
func queryInbox(ctx context.Context, db *sql.DB, p *Provider, mbox, lowerUnix int64, now time.Time) ([]model.Item, error) {
	rows, err := db.QueryContext(ctx, `
		...
		WHERE m.mailbox = ? AND m.deleted = 0 AND m.date_received >= ?
		ORDER BY m.date_received DESC`, mbox, lowerUnix)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&rowid, &received, &address, &comment, &subjectPrefix, &subject, &summary, &msgID); err != nil {
			return nil, fmt.Errorf("scan message: %w", err)
		}
		items = append(items, p.toItem(rowid, received, address, comment, subjectPrefix, subject, summary, msgID, now))
	}
	if err := rows.Err(); err != nil {
...
// used by Body to find the .emlx file. ok is false when no row matches.
func rowidByMessageID(ctx context.Context, db *sql.DB, messageID string) (int64, bool, error) {
	var id int64
	err := db.QueryRowContext(ctx, `
		SELECT m.ROWID
		FROM messages m
		JOIN message_global_data g ON g.ROWID = m.global_message_id
		WHERE g.message_id_header = ? LIMIT 1`, messageID).Scan(&id)
	if errors.Is(err, sql.ErrNoRows) {
		return 0, false, nil
	}
	if err != nil {
...
// id so re-fetches upsert in place. The body is loaded lazily by Body.
func (p *Provider) toItem(rowid int64, received sql.NullInt64, address, comment, subjectPrefix, subject, summary, msgIDHeader string, now time.Time) model.Item {
